...
export async function recordWorkflow(
  ...
  const session = await ensureSession(browser, config);  // login once, reused via storageState
  const vp = config.viewport || {width: 1920, height: 1080};

  // Use storageState if available — navigate directly, no login loop
  const ctx = session
  ...
  try {
    ...
    // Verify auth — if redirected to login, warn but continue with steps anyway
    const onLogin = await page.$('input[type="password"]');
    if (onLogin) {
      console.warn(`    ⚠️  Session restore may have failed (showing login page)`);
    }
