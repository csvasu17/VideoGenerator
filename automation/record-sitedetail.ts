#!/usr/bin/env node
/**
 * record-sitedetail.ts — Records ONLY the site-detail clip using the same infrastructure as record.ts
 *
 * Usage:  npx ts-node --project tsconfig.scripts.json automation/record-sitedetail.ts
 */
...
async function main() {
  ...
  console.log(`\n🎬 Re-recording site-detail clip only`);
  console.log(`   URL:  ${APP_URL}`);
  console.log(`   Out:  public/projects/${PROJECT_ID}/recordings/\n`);

  fs.mkdirSync(RECORDINGS_DIR, {recursive: true});

  // Load full workflows config and filter to just site-detail
  const workflowsPath = path.join(ROOT, 'projects', PROJECT_ID, 'config', 'workflows');
  const mod      = require(workflowsPath);
  const wfConfig = mod.default ?? mod[Object.keys(mod).find((k: string) => k.toLowerCase().includes('workflow')) ?? ''] ?? null;
  ...
  // Filter to ONLY the site-detail clip
  const original   = wfConfig.clips;
  const targetClip = original.find((c: {id: string}) => c.id === 'site-detail');
  if (!targetClip) {
    console.error('site-detail clip not found in workflows config');
    process.exit(1);
  }
  wfConfig.clips = [targetClip];
  ...
  // Update manifest — merge with existing, overwriting just site-detail
  ...
  console.log(`Done. site-detail frames: ${clips[0]?.durationInFrames}`);
}
